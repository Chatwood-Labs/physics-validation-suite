...
from .legacy_v2_details import DetailRow, project_report_details
...
def _state_class(value: Any) -> str:
    ...
    return state if state in {"pass", "warn", "fail", "error", "skip"} else "unknown"


def _detail_rows(rows: tuple[DetailRow, ...]) -> str:
    rendered: list[str] = []
    for row in rows:
        value = escape(row.value).replace("\n", "<br>")
        if row.code:
            value = f"<code>{value}</code>"
        rendered.append(f"<tr><th>{escape(row.label)}</th><td>{value}</td></tr>")
...
def render_html(envelope: dict[str, Any], output_path: Path) -> None:
    ...
    for check in checks:
        check_rows.append(
            ...
            f'<td><span class="state state-{_state_class(check["status"])}">'
    ...
    for check in details.checks:
        reference_row = _row("Reference ID", check.reference_id, code=True)
        audit_blocks.append(
            '<article class="audit-check">'
            '<div class="audit-heading">'
            f"<h3><code>{escape(check.id)}</code></h3>"
            f'<span class="state state-{_state_class(check.status)}">'
            f"{escape(check.status)}</span></div>"
            '<table class="kv audit-meta">'
            f"{_row('Type', check.type)}"
            f"{_row('Required', check.required)}"
            f"{_row('Finding', check.summary)}"
            f"{reference_row}"
            "</table>"
            '<h4>Declaration</h4><table class="kv audit-detail">'
            f"{_detail_rows(check.declaration)}</table>"
            '<h4>Effective criterion</h4><table class="kv audit-detail">'
            f"{_detail_rows(check.criterion)}</table>"
            '<h4>Recorded observation</h4><table class="kv audit-detail">'
            f"{_detail_rows(check.observation)}</table>"
            "</article>"
        )
    ...
    for reference in details.references:
        reference_blocks.append(
            ...
            f"<h3><code>{escape(reference.id)}</code></h3>"
            '<table class="kv reference-detail">'
            f"{_detail_rows(reference.rows)}</table>"
            "</article>"
        )
...
.state-unknown{{color:var(--ink)}} .audit-check,.reference{{padding:18px 0;border-bottom:1px solid var(--line)}}
.audit-heading{{display:flex;align-items:baseline;justify-content:space-between;gap:20px}} .audit-heading h3{{margin:0}}
.audit-check h4{{margin:18px 0 6px;color:var(--muted);font-size:12px;text-transform:uppercase;letter-spacing:.04em}}
.audit-detail td,.reference-detail td{{white-space:pre-wrap;overflow-wrap:anywhere;word-break:break-word}}
.audit-detail th,.reference-detail th{{width:260px;text-transform:none;letter-spacing:0}}
...
@media print{{body{{background:white}}main{{margin:0;box-shadow:none}}.audit-appendix{{break-before:page}}.audit-heading,h2,h3,h4{{break-after:avoid}}tr{{break-inside:avoid}}}}
</style>
</head>
<body><main>
<header>
...
{_row("Scientific finding", details.finding.display, code=details.finding.finding_id is not None)}
</table></section>
...
<section class="audit-appendix"><h2>Check audit appendix</h2>{"".join(audit_blocks)}</section>
...
</table><h3>Scientific finding identity</h3><table class="kv audit-detail">{_detail_rows(details.finding.rows)}</table>
<p>This rendering is human-readable. <code>evidence.json</code> is authoritative. Verify the complete package with <code>pvs verify</code>.</p></section>
<footer><div>PVS evidence {escape(integrity["digest"][:16])} - generated from the authoritative evidence record.</div>
<div>Scientific finding: <code>{escape(details.finding.display)}</code></div></footer>
</main></body></html>"""
