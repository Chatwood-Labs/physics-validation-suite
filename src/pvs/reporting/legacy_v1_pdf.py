...
def render_pdf(envelope: dict[str, Any], output_path: Path) -> None:
    ...
    def footer(pdf: canvas.Canvas, document: SimpleDocTemplate) -> None:
        ...
        pdf.setFont("PVS-Vera", 7)
        pdf.drawString(18 * mm, 9.5 * mm, f"PVS - {evidence_id}")
        pdf.drawRightString(width - 18 * mm, 9.5 * mm, f"Page {document.page}")
        pdf.restoreState()
    ...
    story.extend([paragraph("Checks", "PVSHeading"), check_table, Spacer(1, 5 * mm)])
            ...
            story.append(
                KeepTogether(
                    [
                        ...
                        paragraph(reference["citation"]),
                        paragraph(f"Locator: {_safe(reference['locator'])}", "PVSCode"),
                        paragraph(f"Source location: {_safe(reference.get('source_location'))}"),
                        Spacer(1, 4 * mm),
                    ]
                )
            )
    ...
    doc.build(story, onFirstPage=footer, onLaterPages=footer, canvasmaker=_InvariantCanvas)
