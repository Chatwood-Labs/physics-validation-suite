...
def _flatten(value: Any, path: tuple[str, ...], collector: _RowCollector) -> None:
    ...
    if isinstance(value, dict):
        if set(value) == {"status", "value"} and value["status"] == "AVAILABLE":
            collector.append(path, _scalar_text(value["value"]), code=True)
            return
        if set(value) == {"status", "reason"} and value["status"] == "UNAVAILABLE":
            reason = {"OVERFLOW": "binary64 overflow", "ZERO_REFERENCE": "zero reference"}.get(
                str(value["reason"]), str(value["reason"])
            )
            collector.append(path, f"Unavailable ({reason}); supplementary diagnostic", code=False)
            return
        if not value:
