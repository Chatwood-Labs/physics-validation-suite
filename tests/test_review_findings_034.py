"""Cross-platform regression gates from review of the PVS 0.3.4 release bundle.
...
The original review is preserved verbatim in tests/fixtures/review_034.py.
This port writes explicit UTF-8 bytes: text-mode writes translate LF to CRLF
on Windows, which would contradict the original tests' raw-hash expectations.
The assertions and race interleaving are unchanged. No production files are changed.
"""
...
def test_load_case_binds_raw_hash_to_the_parsed_bytes(
    ...
    path = tmp_path / "pvs.yaml"
    path.write_bytes(texts["subject-A"].encode("utf-8"))
